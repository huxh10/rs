...
void init_rs_wo_sgx(uint32_t num, as_conf_t *p_as_conf)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        memcpy(g_p_policies[i].import_policy, p_as_conf[i].import_policy, num * sizeof(uint32_t));
        g_p_policies[i].export_policy = malloc(num * num * sizeof *g_p_policies[i].export_policy);
        memcpy(g_p_policies[i].export_policy, p_as_conf[i].export_policy, num * num *sizeof(uint32_t));
    }
}
